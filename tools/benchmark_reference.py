...
def plot_comparison(local_results: Dict, ref_results: Dict, output_dir: Path):
    ...
    plt.hist(local_results["response_times"], bins=50, alpha=0.5, label="Local")
    plt.hist(ref_results["response_times"], bins=50, alpha=0.5, label="Reference")
...
def main():
    ...
    with open(report_path, "w") as f:
        ...
        f.write("-----------------\n")
        f.write(f"Average Response Time: {statistics.mean(local_results['response_times']):.2f}ms\n")
        f.write(f"Success Rate: {local_results['success_rate']:.1f}%\n")
        f.write(f"Throughput: {local_results['throughput']:.1f} requests/second\n\n")
        
        f.write("Reference Server Results:\n")
        f.write("----------------------\n")
        f.write(f"Average Response Time: {statistics.mean(ref_results['response_times']):.2f}ms\n")
        f.write(f"Success Rate: {ref_results['success_rate']:.1f}%\n")
        f.write(f"Throughput: {ref_results['throughput']:.1f} requests/second\n\n")
        
        # Calculate performance difference
        time_diff = (statistics.mean(local_results['response_times']) - 
                    statistics.mean(ref_results['response_times']))
        time_diff_pct = (time_diff / statistics.mean(ref_results['response_times'])) * 100
        ...
        if time_diff < 0:
            ...
            f.write("✓ Response times are significantly faster than reference\n")
    ...
    console.print(f"\n[green]Benchmark complete! Results saved to {output_dir}")
